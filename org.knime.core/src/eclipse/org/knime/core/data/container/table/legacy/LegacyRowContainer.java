...
import org.knime.core.data.container.table.legacy.LegacyMappings.LegacyMapper;
...
import org.knime.core.data.table.store.TableChunkStoreUtils;
import org.knime.core.data.table.store.domain.Domain;
import org.knime.core.data.table.store.domain.DomainTableChunkStore;

/**
 *
 * @author Christian Dietz
 */
class LegacyRowContainer implements RowContainer {
    ...
    private final DomainTableChunkStore m_store;
    ...
    LegacyRowContainer(final int tableId, final DataTableSpec spec, final File file,
        ...
        m_store =
            new DomainTableChunkStore(TableChunkStoreUtils.cache(factory.createWriteStore(m_mapping.getSchema(), file)),
                new LegacyDomainConfig());
    ...
    public void addRowToTable(final DataRow row) {
        ...
        for (int i = 0; i < m_values.length; i++) {
            m_values[i].setDataCell(row.getCell(i));
        }
        m_size++;
    }
    ...
    public void close() {
        try {
            ...
            final DataColumnSpec[] colSpecs = new DataColumnSpec[m_dataTableSpec.getNumColumns()];
            final int offset = (m_rowKeyWriteValue == null ? 0 : 1);
            for (int i = 0; i < colSpecs.length; i++) {
                final DataColumnSpecCreator specCreator = new DataColumnSpecCreator(m_dataTableSpec.getColumnSpec(i));
                Domain result = m_store.getResultDomain(i + offset);
                if (result != null) {
                    @SuppressWarnings("unchecked")
                    LegacyMapper<?, Domain> mapper = (LegacyMapper<?, Domain>)LegacyMappings.PRIMITIVE_MAPPINGS
                        .get(m_dataTableSpec.getColumnSpec(i).getType());
                    specCreator.setDomain(mapper.convert(result));
                }
                colSpecs[i] = specCreator.createSpec();
            }

            m_table = new LegacyTmpTableStoreTable(m_factory, m_tableId, new DataTableSpec(colSpecs), m_store,
                m_mapping, m_size);
        } catch (Exception ex) {
            // TODO Handle
