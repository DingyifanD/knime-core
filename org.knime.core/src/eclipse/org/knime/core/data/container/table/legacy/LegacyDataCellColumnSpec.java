...
class LegacyDataCellColumnSpec<C extends ColumnChunk> implements ColumnSpec<BinarySupplChunk<C>> {
    ...
    private LegacyMapper<C, ?> m_info;

    private DataCellSerMap m_serMap;

    public LegacyDataCellColumnSpec(final DataCellSerMap serMap, final LegacyMapper<C, ?> info) {
        m_info = info;
        m_serMap = serMap;
    }
    ...
    static class DataCellColumnAccess<C extends ColumnChunk> extends AbstractColumnChunkAccess<BinarySupplChunk<C>>
        ...
        private LegacyMapper<C, ?> m_mapper;

        private final DataCellSerMap m_serMap;

        private boolean m_hasBinary;

        public DataCellColumnAccess(final DataCellSerMap serMap, final LegacyMapper<C, ?> mapper) {
            m_serMap = serMap;
            m_cellType = mapper.getCellType();
            m_mapper = mapper;
        }
        ...
        public void load(final BinarySupplChunk<C> data) {
            ...
            m_hasBinary = m_binaryData.getNumValues() > 0;
        ...
        public void setDataCell(final DataCell cell) {
            if (cell.getClass() == m_cellType) {
                m_mapper.set(m_index, cell, m_valueData);
        ...
        public DataCell getDataCell() {
            if (!m_valueData.isMissing(m_index)) {
                if (!m_hasBinary || m_binaryData.isMissing(m_index)) {
                    return m_mapper.get(m_index, m_valueData);
                } else {
            ...
            } else if (m_hasBinary && !m_binaryData.isMissing(m_index)) {
                try {
