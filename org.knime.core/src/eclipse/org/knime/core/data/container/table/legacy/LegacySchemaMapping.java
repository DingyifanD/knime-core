 ...
 */
public class LegacySchemaMapping implements TableSchemaMapping {
    ...
    private TableSchema map(final DataTableSpec spec, final boolean isRowKey) {
        ...
        for (int i = 0; i < spec.getNumColumns(); i++) {
            final LegacyMapper<?, ?> mapper = LegacyMappings.PRIMITIVE_MAPPINGS.get(spec.getColumnSpec(i).getType());
            if (mapper != null) {
                columnSpec[i + offset] = new LegacyDataCellColumnSpec<>(m_map, mapper);
            } else {
