...
public class LazyTableStoreTable extends AbstractFastTable {
    ...
    public LazyTableStoreTable(final LoadContext context) throws InvalidSettingsException {
        super(-1, context.getTableSpec(), getSchemaFromSettings(context),
            context.getSettings().getLong(FAST_TABLE_CONTAINER_SIZE));
        m_readTask = new AccessTask(context.getDataFileRef(),
            FastTableUtil.createInstance(context.getSettings().getString(FAST_TABLE_CONTAINER_TYPE)), getSchema());
    }
     ...
     * @throws InvalidSettingsException
     */
    private static TableSchema getSchemaFromSettings(final LoadContext context) throws InvalidSettingsException {
        NodeSettingsRO settings = context.getSettings();
        TableSchemaMapping mapping = FastTableUtil.createInstance(settings.getString(FAST_TABLE_MAPPING_TYPE));
